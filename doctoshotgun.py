...
class VaccineStore:
    ...
    def get_vaccine_ids(self):
        self.vaccines.keys()

    def getVaccine(self, id):
        return self.vaccines[id]

    def addVaccine(self, id: str, vaccine_name: str):
        self.vaccines[id] = Vaccine(vaccine_name)
...
class Doctolib(LoginBrowser):
    ...
    def find_centers(self, where, motives=None, page=1):
        if motives is None:
            motives = self.vaccine_motives.get_vaccine_ids()
        for city in where:
            try:
    ...
    def try_to_book(self, center, vaccine_list, start_date, end_date, only_second, only_third, dry_run=False):
        ...
        for vaccine in vaccine_list:
            motives_id[vaccine] = self.page.find_motive(
                r'.*({})'.format(vaccine), singleShot=(vaccine == self.vaccine_motives.getVaccine(self.KEY_JANSSEN).name or only_second or only_third))
...
class DoctolibDE(Doctolib):
    ...
    vaccine_motives = VaccineStore()
    vaccine_motives.addVaccine(KEY_PFIZER, 'Pfizer')
    vaccine_motives.addVaccine(KEY_PFIZER_SECOND, 'Zweit.*Pfizer|Pfizer.*Zweit')
    vaccine_motives.addVaccine(KEY_PFIZER_THIRD, 'Dritt.*Pfizer|Pfizer.*Dritt')
    vaccine_motives.addVaccine(KEY_MODERNA, 'Moderna')
    vaccine_motives.addVaccine(KEY_MODERNA_SECOND, 'Zweit.*Moderna|Moderna.*Zweit')
    vaccine_motives.addVaccine(KEY_MODERNA_THIRD, 'Dritt.*Moderna|Moderna.*Dritt')
    vaccine_motives.addVaccine(KEY_JANSSEN, 'Janssen')
    vaccine_motives.addVaccine(KEY_ASTRAZENECA, 'AstraZeneca')
    vaccine_motives.addVaccine(KEY_ASTRAZENECA_SECOND, 'Zweit.*AstraZeneca|AstraZeneca.*Zweit')
...
class DoctolibFR(Doctolib):
    ...
    vaccine_motives = VaccineStore()
    vaccine_motives.addVaccine(KEY_PFIZER, 'Pfizer')
    vaccine_motives.addVaccine(KEY_PFIZER_SECOND, '2de.*Pfizer')
    vaccine_motives.addVaccine(KEY_PFIZER_THIRD, '3e.*Pfizer')
    vaccine_motives.addVaccine(KEY_MODERNA, 'Moderna')
    vaccine_motives.addVaccine(KEY_MODERNA_SECOND, '2de.*Moderna')
    vaccine_motives.addVaccine(KEY_MODERNA_THIRD, '3e.*Moderna')
    vaccine_motives.addVaccine(KEY_JANSSEN, 'Janssen')
    vaccine_motives.addVaccine(KEY_ASTRAZENECA, 'AstraZeneca')
    vaccine_motives.addVaccine(KEY_ASTRAZENECA_SECOND, '2de.*AstraZeneca')
...
class Application:
    ...
    def main(self, cli_args=None):
        ...
        vaccine_list = [docto.vaccine_motives.getVaccine(motive).name for motive in motives]
