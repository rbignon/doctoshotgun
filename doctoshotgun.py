...
class IBASEURL(metaclass=ABCMeta):

    @abstractstaticmethod
    def get_BASEURL():
        """ gets BaseURL of the country(germany or france)"""


class DoctolibDE(Doctolib,IBASEURL):
...
class DoctolibFR(Doctolib,IBASEURL):
...
class compositeBaseURL(IBASEURL):
    strings = ["https://www.doctolib.de", "https://www.doctolib.fr"]
    def __init__(self):
        self.child_BASEURL = []
    def replace_BASEURL(self, newBASEURL)
        for x in strings:
            new_string = x.replace("https://www.doctolib.de" | "https://www.doctolib.fr", newBASEURL)
            self.child_BASEURL.new_strings.append(newBASEURL)
    def get_BASEURL(self):
        return self.child_BASEURL
